...
class NN():
    ...
    def backward(self, X, y): # backward propagation
        ...
        layer_error = self.forward(X) * (np.array(1) - self.forward(X)) * (np.array(y) - self.forward(X))
        # Weight updation- W = W + alpha * (H * ((y-OP) * (OP * (1 - OP))))
        self.W[-1] += self.lr * (np.dot(np.matrix(self.forward(X,hidden_layers)).transpose(), layer_error).transpose())

        # there are hidden_layers + 1 number of weight matrices, hidden_layers number of matrices if we calculate first one out of loop
        for step in range(2,hidden_layers+2): # the loop runs for hiden_layers number of iterations
            layer_error = np.array(self.forward(X, hidden_layers+2-step)) * (np.array(1) - np.array(self.forward(X, hidden_layers+2-step))) * np.dot(self.W[-step+1],np.matrix(layer_error).transpose())
            self.W[-step] += self.lr * (np.dot(np.matrix(self.forward(X,hidden_layers+1-step)).transpose(), layer_error).transpose())
